...
def prepare_multisplit_workspaces(source_dir, base_workspace_dir):
    ...
    try:
        start_frame = int(input("Global Sequence Start Frame (ex. 0): ") or 0)
        end_frame = int(input(f"Global Sequence End Frame (ex. {total_images}): ") or total_images)
        
        pct_str = input("Enter fixed overlap (float/integer)%").replace('%', '')

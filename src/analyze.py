...
GLOBAL_SPARSE_DIR = Path(r"C:\Users\neela\OneDrive\Documents\Github\colmap-split-merge\colmap_workspace\full_sequence_0_to_200\sparse\0")
SOURCE_IMAGE_DIR = Path(r"C:\Users\neela\OneDrive\Documents\Github\colmap-split-merge\dataset")
OUTPUT_ANALYSIS_DIR = Path(r"C:\Users\neela\OneDrive\Documents\Github\colmap-split-merge\colmap_workspace\analysis")

def analyze_scene_density():
    if not GLOBAL_SPARSE_DIR.exists():
        print(f"Error: Sparse model directory does not exist: {GLOBAL_SPARSE_DIR}")
        return
    ...
    if not all_color_images:
        all_color_images = sorted([
            f.name for f in SOURCE_IMAGE_DIR.iterdir()
            if f.is_file() and f.suffix.lower() in [".png", ".jpg", ".jpeg"]
        ])

    if not all_color_images:
        print(f"Error: No images found in source directory: {SOURCE_IMAGE_DIR}")
        return
    ...
    print(f"Saved point data profiles to: {json_path}")
    ...
    print("\nSample Chronological Frame Benchmarks:")
    ...
    plt.hist(counts, bins=40, color='royalblue', edgecolor='royalblue', alpha=0.8, label='Images')
    ...
    plt.show()
    print(f"Histogram graphic generated and saved to: {plot_path}")

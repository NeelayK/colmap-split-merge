...
def execute_adaptive_batch_reconstruction(base_dir):
    ...
    if not dataset_dirs:
        print(f"No valid experiment directories found in {base_path}.")
        return

    print(f"Found {len(dataset_dirs)} workspace configurations. Starting batch processing...")
    ...
    for workspace_dir in sorted(dataset_dirs):
        ...
        try:
            ...
            print(f"\nPipeline execution failure on workspace context {workspace_dir.name}. Skipping to next configuration.")
            continue 
    ...
    print("\n" + "="*70)
    print(" ADAPTIVE BATCH RECONSTRUCTION COMPLETE ")
    print("="*70)
...
if __name__ == "__main__":
    ...
    execute_adaptive_batch_reconstruction(base_workspace_dir)